	...
	case GUI_MSG_CREATE:
		{
			...
			GUI_LyrWinSetSta(home_para->lyr_mmenu, GUI_LYRWIN_STA_ON);		//GUI_LYRWIN_STA_ON
			GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_ON);			//GUI_LYRWIN_STA_ON
		...
		{					
			...
			if (1/*home_para->root_type != root_type*/)
			{
				...
				if (home_para->focus_id == ID_HOME_MOVIE || home_para->focus_id == ID_HOME_PHOTO ||
					home_para->focus_id == ID_HOME_MUSIC|| home_para->focus_id == ID_HOME_EXPLORER)		// || home_para->focus_id == ID_HOME_EBOOK
