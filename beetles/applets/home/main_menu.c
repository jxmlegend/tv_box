...
//	com_memdev_create(mmenu_attr->layer);
    GUI_SetFont(mmenu_attr->font);
    GUI_UC_SetEncodeUTF8();
    GUI_SetBkColor(0);

	__msg("smenu_attr->tvmenu_font=%x\n",mmenu_attr->font);
	...
	case GUI_MSG_CREATE:
		{
			...
			mmenu_attr->first_item = 0;
			mmenu_attr->focus_txt_color = GUI_YELLOW;
			mmenu_attr->unfocus_txt_color = GUI_WHITE;
            mmenu_attr->res_init = EPDK_FALSE;
