...
static void __app_home_restore(__gui_msg_t* msg)
{
	...
	{
		__msg("**********TV  to home_____67**********\n");
		root_ctrl->h_app_home = app_home_create(root_ctrl->root_para);		//csq
//		root_ctrl->h_app_home = app_tv_create(root_ctrl->root_para);
		__app_set_focus_child(root_ctrl->h_app_home, 0);
	}
}
				...
				case SWITCH_TO_OTHER_APP:
				{
					...
					{						
						...
						{                            
							...
							__msg("**********enter TV**********\n");							
							root_ctrl->root_para->h_parent= GUI_WinGetParent(root_ctrl->h_app_home);
//							root_ctrl->h_app_fm=  app_fm_create(root_para);			//csq
...
//[led_para]
//TV_LED	        = port:PE12<1><default><default><default>
...
//			GUI_WinSetFocusChild(root_ctrl->h_app_home);
		...
		case GUI_MSG_KEY:
		{
            ...
            {
                if(KEY_UP_ACTION == msg->dwAddData2
                    &&GUI_MSG_KEY_LEFT == msg->dwAddData1)
                {
