...
static __s32 __movie_ctrl_para_init(movie_ctrl_t* movie_ctrl)
{
...

	{
		...
		vedio_rect.x = 0;
		vedio_rect.y = 0;
		vedio_rect.width = screen_width;		//csq
		vedio_rect.height = screen_height;
//		vedio_rect.width = 700;
//		vedio_rect.height = 480;
...
static __s32 __movie_mute_scene_create(movie_ctrl_t* movie_ctrl)
{
	__s32 ret;	
    ...
    if (NULL == movie_ctrl->mute_scene)
	{
		movie_mute_create_para_t create_para;
		eLIBs_memset(&create_para, 0, sizeof(movie_mute_create_para_t));
		create_para.hparent = movie_ctrl->movie_manager;
		create_para.scene_id = MOVIE_MUTE_ID;
		movie_ctrl->mute_scene = movie_mute_scene_create(&create_para);
		if (NULL == movie_ctrl->mute_scene)
		{
			__msg("movie_mute_scene_create fail...\n");
			return EPDK_FAIL;
		}
        movie_mute_scene_set_focus(movie_ctrl->mute_scene);
	}
	
	return EPDK_OK;
}

static __s32 __movie_mute_scene_delete(movie_ctrl_t* movie_ctrl)
{
	__s32 ret;	
	...
	{
		if (movie_ctrl->mute_scene)
		{
			movie_mute_scene_delete(movie_ctrl->mute_scene);
			movie_ctrl->mute_scene = NULL;
		}
	}
...
static __s32 __movie_has_sub_scene(movie_ctrl_t* movie_ctrl, __u32 sub_id)
{
    ...
    __msg("__movie_has_sub_scene, child = %d \n", child);
    while( child != NULL )
	{				
		child_id = GUI_WinGetItemId(child);
		__msg("__movie_has_sub_scene, child_id = %d \n", child_id);
		if ((child_id & sub_id))
		{
			return EPDK_TRUE;				
		}
	...
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
			...
			__movie_ctrl_para_init(movie_ctrl);
		  	...
		  	if(movie_muteflag==1)
			{
				__movie_mute_scene_create(movie_ctrl);
			}  						
			...
			if (KEY_UP_ACTION == msg->dwAddData2)
			{
				...
				else if (GUI_MSG_KEY_ENTER == last_key)
				{
					...
					if(EPDK_FALSE == ret)//如果没有除字幕以外的子场景，则处理暂停、继续播放
						...
						else if((CEDAR_STAT_FF == fsm_sta)||(CEDAR_STAT_RR == fsm_sta))
						{
	        					__msg("before robin_set_cmd_play\n");
	        					robin_set_cmd_play();                           
	        					__msg("after robin_set_cmd_play\n");
					...
					{
						__cedar_status_t fsm_sta;
						__movie_delete_subscene_by_id(movie_ctrl, MOVIE_VOLUME_ID);
				...
				else if(GUI_MSG_KEY_MUTE == last_key)
				{
					movie_set_mute();
					if(movie_muteflag==1)
					{
				...
				else if(GUI_MSG_KEY_QPre == last_key)
				{
	                    		__s32 ret;
				...
				else if(GUI_MSG_KEY_Qnex == last_key)
				{
	                    		__s32 ret;
				...
				{	
				    	...
				    	ret = __movie_has_sub_scene(movie_ctrl, MOVIE_SUB_SCENE_TYPE_ALL&(~MOVIE_SUB_SHOW_ID)&(~MOVIE_PLAYSTA_ID)&(~MOVIE_MUTE_ID));
                ...
                else if(GUI_MSG_KEY_LEFT == last_key)
                {
					...
					else
					{
						...
						{
		                    			ret = __movie_has_sub_scene(movie_ctrl, MOVIE_VOLUME_ID);
							ret2 =  __movie_has_sub_scene(movie_ctrl, MOVIE_SUB_SHOW_ID);	
							if(EPDK_FALSE == ret2)
							{
								if (EPDK_FALSE == ret)
								{							
...
/*		screen_size.width	 = 720;
		screen_size.height	 = 576;		
