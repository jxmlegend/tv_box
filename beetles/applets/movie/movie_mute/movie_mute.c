...
/***********************************************************************************************************
	建立图层
************************************************************************************************************/
static H_LYR __moviemute_8bpp_layer_create(RECT *rect, __s32 pipe)
{
	__s32           screen_width;
    __s32 			screen_height;
	H_LYR layer = NULL;
...
static __s32 __moviemute_init_ui(void)
{
	movie_mute_uipara_t* ui_para;
	
	ui_para = movie_mute_get_uipara(GUI_GetScnDir());
	
	if (NULL == ui_para)
	{				
		__msg("movie_mute_get_uipara fail...\n");
		return EPDK_FAIL;
	}		
	
	{
		...
		{
			for (j = 0 ; j < MOVIE_MUTE_MAX_ICON_RES_NUM ; j++)
			{
...
static __s32 __moviemute_update_moviemute_ui(movie_mute_scene_t* scene_para)
{
	movie_mute_uipara_t* ui_para;
...
static __s32 __moviemute_deinit_ui(void)
{
	movie_mute_uipara_t* ui_para;
...
static __s32 __moviemute_install_hide_timmer(movie_mute_scene_t* scene_para)
{
	if (!scene_para)
	{
		__err("invalid para...\n");
...
static __s32 __moviemute_reset_hide_timmer(movie_mute_scene_t* scene_para)
{
	if (!scene_para)
	{
		__err("invalid para...\n");
...
static __s32 __moviemute_uninstall_hide_timmer(movie_mute_scene_t* scene_para)
{
	if (!scene_para)
	{
		__err("invalid para...\n");
...
static __s32  moviemute_key_proc(__gui_msg_t *msg)
{
	static __s32 last_key = -1;
	movie_mute_scene_t* scene_para;

	scene_para = GUI_WinGetAttr(msg->h_deswin);		

    	__msg("app_movie_mute_key->dwAddData1=%d, msg->dwAddData2=%d\n"
	...
	if (KEY_UP_ACTION == msg->dwAddData2)
	{
		...
		__msg("**********movie_mute_key_proc**********\n");
	}
	else
	{
		__msg("**********moviem_mute_key_proc 2**********\n");
...
static __s32 __moviemute_proc(__gui_msg_t* msg)
{
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
			...
			__msg("__moviemute_proc GUI_MSG_CREATE begin\n");
...
//			com_set_palette_by_id(ID_MOVIE_PAL_BMP);//设置调色板
			...
			__moviemute_update_moviemute_ui(scene_para);

//			__moviemute_install_hide_timmer(scene_para);

			__msg("__moviemute_proc GUI_MSG_CREATE end\n");
			return EPDK_OK;
		}
		case GUI_MSG_DESTROY:
		{
			...
			__msg("__moviemute_proc GUI_MSG_DESTROY begin\n");
			...
			__moviemute_uninstall_hide_timmer(scene_para);
			__moviemute_deinit_ui();

			__msg("__moviemute_proc GUI_MSG_DESTROY end\n");
			return EPDK_OK;
		}
		case GUI_MSG_KEY:
		{		
			moviemute_key_proc(msg);
			return EPDK_OK;
		}
		case GUI_MSG_TOUCH:
		{			
			...
			__msg("__moviemute_proc GUI_MSG_TOUCH begin\n");
			...
			{
				__moviemute_reset_hide_timmer(scene_para);
//				__moviemute_change_sta(scene_para);
//				movie_cmd2parent(msg->h_deswin, movie_mute_scene_msg_clicked, scene_para->moviemute, 0);				
			}

			__msg("__moviemute_proc GUI_MSG_TOUCH end\n");
			return EPDK_OK;
		}
		case GUI_MSG_PAINT:
		{
			...
			__msg("__moviemute_proc GUI_MSG_PAINT begin\n");
		...
		case GUI_MSG_TIMER:
		{
			...
			__msg("__moviemute_proc GUI_MSG_TIMER begin\n");
			...
			__msg("__moviemute_proc GUI_MSG_TIMER end\n");
		...
		case GUI_MSG_CLOSE:
		{
			__msg("__moviemute_proc GUI_MSG_CLOSE begin\n");
			movie_cmd2parent(msg->h_deswin, movie_mute_scene_msg_timeout, 0, 0);
			__msg("__moviemute_proc GUI_MSG_CLOSE end\n");
			break;
		}
		default:
		{
...
void* movie_mute_scene_create(movie_mute_create_para_t *create_para)
{
	...
	movie_mute_scene_t *sence_para;	
	movie_mute_uipara_t* ui_para;
	RECT lyr_rect;

	sence_para = (movie_mute_scene_t *)esMEMS_Balloc(sizeof(movie_mute_scene_t));
	if (!sence_para)
	{
	...
	eLIBs_memset(sence_para, 0, sizeof(movie_mute_scene_t));
	...
	if (NULL == ui_para)
	{
		esMEMS_Bfree(sence_para, sizeof(movie_mute_scene_t));
		sence_para = NULL;
	...
	sence_para->hlyr = __moviemute_8bpp_layer_create(&lyr_rect, ui_para->pipe);
	...
	framewin_para.name			=	"movie_mute_win",
...
//	framewin_para.hHosting		=  h_parent;
	...
	framewin_para.FrameWinProc	= (__pGUI_WIN_CB)esKRNL_GetCallBack((__pCBK_t)__moviemute_proc);
...
__s32 movie_mute_scene_delete(void* handle)
{
	...
	scene_para = (movie_mute_scene_t*)handle;
	...
	eLIBs_memset(scene_para, 0, sizeof(movie_mute_scene_t));//防止重复释放
	esMEMS_Bfree(scene_para, sizeof(movie_mute_scene_t));
