...
{
	tv_volume_uipara_t* ui_para;
	
	ui_para = tv_volume_get_uipara(GUI_GetScnDir());
	
	if (NULL == ui_para)
	{				
		__msg("tv_volume_get_uipara fail...\n");
		return EPDK_FAIL;
	}	

	{
		...
		for (i = 0 ; i < TV_VOLUME_MAX_ICON_RES_NUM ; i++)
		{			
		...
		for (i = 0 ; i < TV_VOLUME_MAX_ICON_RES_NUM ; i++)
		{			
			if (ui_para->uipara_volume_text.res_id[i])
			{
				ui_para->uipara_volume_text.res_hdl[i] = dsk_theme_open(ui_para->uipara_volume_text.res_id[i]);
				if (!ui_para->uipara_volume_text.res_hdl[i])
				{
					__msg("dsk_theme_open fail...\n");
				}
			}
	...
	
	{
		...
		for (i = 0 ; i < tv_volume_icon_num ; i++)
		{
			for (j = 0 ; j < TV_VOLUME_MAX_ICON_RES_NUM ; j++)
			{
...
static __s32 __volume_deinit_ui(void)
{
	...
	
	{
		...
		for (i = 0 ; i < TV_VOLUME_MAX_ICON_RES_NUM ; i++)
		{
			if (ui_para->uipara_volume_text.res_hdl[i])
			{
				dsk_theme_close(ui_para->uipara_volume_text.res_hdl[i]);
				ui_para->uipara_volume_text.res_hdl[i] = NULL;
			}
		}
	}
...
static __s32 __volume_draw_bg(tv_volume_scene_t* scene_para)
{
	tv_volume_uipara_t* ui_para;
...
static __s32 __tv_volume_update_volume_ui(tv_volume_scene_t* scene_para)
{
	tv_volume_uipara_t* ui_para;
	...
	//画音量图标
	{
		...
		if (!ui_para->uipara_icon[tv_volume_icon_lvolume].res_hdl[focus])
		{
			__msg("ui_para->uipara_icon[tv_volume_icon_lvolume].res_hdl[focus] is null...\n");
			//com_memdev_delete();
			return EPDK_FAIL;				
		}		
		pbmp = dsk_theme_hdl2buf(ui_para->uipara_icon[tv_volume_icon_lvolume].res_hdl[focus]);
		if (!pbmp)
		{
			__msg("pbmp is null...\n");
			//com_memdev_delete();
        ...
        {
            GUI_RECT gui_rect;
            gui_rect.x0 = ui_para->uipara_icon[tv_volume_icon_lvolume].x;
            gui_rect.y0 = ui_para->uipara_icon[tv_volume_icon_lvolume].y;
            gui_rect.x1 = gui_rect.x0+ui_para->uipara_icon[tv_volume_icon_lvolume].w-1;
            gui_rect.y1 = gui_rect.y0+ui_para->uipara_icon[tv_volume_icon_lvolume].h-1;
            GUI_ClearRect(gui_rect.x0, gui_rect.y0, gui_rect.x1, gui_rect.y1);
        }
		GUI_BMP_Draw(pbmp, ui_para->uipara_icon[tv_volume_icon_lvolume].x, ui_para->uipara_icon[tv_volume_icon_lvolume].y);


		if (!ui_para->uipara_icon[tv_volume_icon_rvolume].res_hdl[focus])
		{
			__msg("ui_para->uipara_icon[tv_volume_icon_rvolume].res_hdl[focus] is null...\n");
			//com_memdev_delete();
			return EPDK_FAIL;				
		}		
		pbmp = dsk_theme_hdl2buf(ui_para->uipara_icon[tv_volume_icon_rvolume].res_hdl[focus]);
		if (!pbmp)
		{
			__msg("pbmp is null...\n");
			//com_memdev_delete();
		...
		GUI_BMP_Draw(pbmp, ui_para->uipara_icon[tv_volume_icon_rvolume].x, ui_para->uipara_icon[tv_volume_icon_rvolume].y);					
	}
	...
	//画文字
	{
		...
		//GUI_ClearRectEx(&gui_rect);
		{
			...
			{			
				if (ui_para->uipara_volume_text.res_hdl[i])
				{
					pbmp = dsk_theme_hdl2buf(ui_para->uipara_volume_text.res_hdl[i]);
					if (!pbmp)
					{
						__msg("pbmp is null...\n");
						return EPDK_FAIL;
					...
					{
						GUI_BMP_Draw(pbmp, ui_para->uipara_volume_text.x, ui_para->uipara_volume_text.y);
					}		
				}						
			}
		}
	...
	//画进度条背景
	{
		...
		if (!ui_para->uipara_icon[tv_volume_icon_prog_bg].res_hdl[0])
		{
			__msg("ui_para->uipara_icon[tv_volume_icon_prog_bg].res_hdl[0] is null...\n");
			//com_memdev_delete();
			return EPDK_FAIL;				
		}		
		pbmp = dsk_theme_hdl2buf(ui_para->uipara_icon[tv_volume_icon_prog_bg].res_hdl[0]);
		if (!pbmp)
		{
			__msg("pbmp is null...\n");
			//com_memdev_delete();
		...
		GUI_BMP_Draw(pbmp, ui_para->uipara_icon[tv_volume_icon_prog_bg].x, ui_para->uipara_icon[tv_volume_icon_prog_bg].y);
	}
		...
		&& scene_para->cur_val <= scene_para->max_val)
	{
		...
		if (!ui_para->uipara_icon[tv_volume_icon_prog_cursor].res_hdl[0])
		{
			__msg("ui_para->uipara_icon[tv_volume_icon_prog_cursor].res_hdl[0] is null...\n");
			//com_memdev_delete();
			return EPDK_FAIL;				
		}		
		pbmp = dsk_theme_hdl2buf(ui_para->uipara_icon[tv_volume_icon_prog_cursor].res_hdl[0]);
		if (!pbmp)
		{
			__msg("pbmp is null...\n");
			//com_memdev_delete();
...

		{
			...
			{
				n = ui_para->uipara_icon[tv_volume_icon_prog_bg].w*(scene_para->cur_val-scene_para->min_val)
					/ui_para->uipara_icon[tv_volume_icon_prog_cursor].w/(scene_para->max_val - scene_para->min_val);
			}

			for (i = 0 ; i < n ; i++)
			{
				GUI_BMP_Draw(pbmp, ui_para->uipara_icon[tv_volume_icon_prog_cursor].x+i*ui_para->uipara_icon[tv_volume_icon_prog_cursor].w
					, ui_para->uipara_icon[tv_volume_icon_prog_cursor].y);
			}
		}		
	}
...
static __s32 __volume_install_hide_timmer(tv_volume_scene_t* scene_para)
{
	if (!scene_para)
	{
		__err("invalid para...\n");
...
static __s32 __volume_reset_hide_timmer(tv_volume_scene_t* scene_para)
{
	if (!scene_para)
	{
		__err("invalid para...\n");
...
static __s32 __volume_uninstall_hide_timmer(tv_volume_scene_t* scene_para)
{
	if (!scene_para)
	{
		__err("invalid para...\n");
...
static __s32 __tv_volume_update_vol(tv_volume_scene_t* scene_para)
{
		GUI_MEMDEV_Handle	draw_mem; 

		draw_mem = GUI_MEMDEV_Create(0, 578,1024,72);
...
static __s32 __tv_volume_proc(__gui_msg_t* msg)
{
...
//						dsk_volume_set(scene_para->cur_val);	
						...
						movie_cmd2parent(msg->h_deswin, tv_volume_scene_msg_volumeset, scene_para->cur_val, 0);
					}
				}
...
//						dsk_volume_set(scene_para->cur_val);	
		...
		case GUI_MSG_CLOSE:
		{
			...
			movie_cmd2parent(msg->h_deswin, tv_volume_scene_msg_timeout, 0, 0);
...
void* tv_volume_scene_create(H_WIN h_parent, tv_volume_create_para_t *create_para)
{
	...
	tv_volume_scene_t *sence_para;	
	tv_volume_uipara_t* ui_para;
	RECT lyr_rect;

	sence_para = (tv_volume_scene_t *)esMEMS_Balloc(sizeof(tv_volume_scene_t));
	if (!sence_para)
	{
	...
	eLIBs_memset(sence_para, 0, sizeof(tv_volume_scene_t));
	...
	if (NULL == ui_para)
	{
		esMEMS_Bfree(sence_para, sizeof(tv_volume_scene_t));
		sence_para = NULL;
...
*/
	...
	framewin_para.name			=	"tv_volume_win",
	framewin_para.dwExStyle		= WS_EX_NONE;
	framewin_para.dwStyle		= WS_NONE|WS_VISIBLE;
	framewin_para.spCaption		= NULL;
	framewin_para.hOwner		= NULL;
	framewin_para.id			= (APP_TV_ID+2);
	framewin_para.hHosting		=  h_parent;
	framewin_para.FrameWinProc	= (__pGUI_WIN_CB)esKRNL_GetCallBack((__pCBK_t)__tv_volume_proc);
...
__s32 tv_volume_scene_delete(void* handle)
{
	...
	scene_para = (tv_volume_scene_t*)handle;
	...
	eLIBs_memset(scene_para, 0, sizeof(tv_volume_scene_t));//防止重复释放
	esMEMS_Bfree(scene_para, sizeof(tv_volume_scene_t));
