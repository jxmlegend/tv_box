...
{	
	...
	{	
		...
		//activity_kill_all_backgrd();		
		activity_finish_all();
		gscene_bgd_set_state(BGD_STATUS_HIDE);			//	BGD_STATUS_SHOW
...
led_flash_rcv_t *led_flash_rcv = NULL;
...
static __s32 __app_request_pins(void)
{
    ...
    if(!g_pe12)
    {
        ...
        ret = esCFG_GetKeyValue("led_para", "TV_LED", (int *)gpio_set, sizeof(user_gpio_set_t)/4);
        if (!ret)
        {
            g_pe12 = esPINS_PinGrpReq(gpio_set, 1);
            if (!g_pe12)
            {
...
{  
	...
	if(!g_pe12)
    	...
    	esPINS_WritePinData(g_pe12, 1, 0);
	return EPDK_OK;
}
...
{  
	 ...
	 if(!g_pe12)
    	...
    	esPINS_WritePinData(g_pe12, 0, 0);
...
{	
	...
	led_flash_rcv = (led_flash_rcv_t *)esMEMS_Malloc(0,sizeof(led_flash_rcv_t));
	if(led_flash_rcv == NULL)
	{
		__msg("dsk_tv_rcv malloc error!\n");
		return EPDK_FAIL;
	}
	eLIBs_memset(led_flash_rcv, 0, sizeof(led_flash_rcv));	
	led_flash_rcv->thread_id = esKRNL_TCreate(__ledflash_thread, (void *)led_flash_rcv, 0x400,KRNL_priolevel3);
...
{	
	...
	if( output == DISP_OUTPUT_TYPE_TV)			//DISP_OUTPUT_TYPE_LCD
	{	
		__here__
		/* 置关屏状态位 */
		init_lock(msg);
...
{	
    ...
    __here__;
	if( (output == DISP_OUTPUT_TYPE_NONE)||(output == DISP_OUTPUT_TYPE_TV) )		//DISP_OUTPUT_TYPE_LCD
	{		
        __here__;
		init_lock(msg);
        __here__;
